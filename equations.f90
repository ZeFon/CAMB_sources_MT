        ...
        if (tau>tau_start_redshiftwindows .or. plot_evolve) then
            ...
            do w_ix = 1, num_redshiftwindows
                ...
                elseif (W%kind == window_counts) then
                    ...
                    sources(3+w_ix)= counts_density_source + counts_redshift_source + CP%eps_GR*(counts_radial_source &
                    ...
                    if (DoRedshiftLensing) &
                        sources(3+W%mag_index+num_redshiftwindows) = CP%eps_WL*phi*W%win_lens(j)*(2-5*W%dlog10Ndm)
