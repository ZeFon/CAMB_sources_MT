    ...
    integer i
    character(LEN=Ini_max_string_len) Zwintype, ZdNdz
    ...
    do i=1, num_redshiftwindows
        ...
        Zwintype = Ini_Read_String('redshift_wintype('//trim(numstr)//')')
        if (Zwintype=='gaussian') then
	...
	!Parameter for non-Gaussianity in bias
    P%fNL    = Ini_Read_Double('fNL')
    P%SCtransferTOT = Ini_Read_Double('normtrans')
	
	!marginalising parameters
	P%eps_WL    = Ini_Read_Double('eps_WL')
	P%eps_GR    = Ini_Read_Double('eps_GR')
	P%eps_IM    = Ini_Read_Double('eps_IM')
	P%eps_gal    = Ini_Read_Double('eps_gal')
