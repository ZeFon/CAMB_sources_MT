    ...
    integer, parameter :: max_redshiftwindows  = 40
   ...
   elseif (Win%wintype == tophat) then
    ...
    type CAMBparams
		...
		real(dl)  :: fNL,SCtransferTOT !nG in bias
		real(dl)  :: eps_WL, eps_GR, eps_IM, eps_gal !marginalization parameters
...
500     if (L< lmax_extrap_highl) &
        ...
        do i = 1, num_redshiftwindows
            ...
            elseif ( RedWin%kind == window_counts) then
                ...
                if (counts_evolve) then
                    ...
                    if (tau < CP%tau0 -0.1) then
 					   if (RedWin%dNdz == im) then
						   !for IM counts_background_z is T_HI
 						   RedWin%comoving_density_ev(j) = back_count_tmp(j,i)*(adot*a)
 					   else 
						   !for galaxy surveys counts_background_z is dN/dz
    ...
    do i = 1, num_redshiftwindows
        ...
        elseif (RedWin%kind == window_counts) then
            ...
            if (counts_evolve) then
                ...
                do ix = jstart, TimeSteps%npoints
					...
					elseif (RedWin%dNdz == im) then
						...
						RedWin%comoving_density_ev(ix) =   CP%eps_gal*(tmp2(ix) / RedWin%comoving_density_ev(ix) &
			...
			!RedWin%comoving_density_ev(ix)=f_evo*H
            ...
            !wingtau =  g*(2/H\chi + Hdot/H^2)  when s=f_evo=0; int_tmp = window/chi
			...
			RedWin%Wingtau(jstart:TimeSteps%npoints) = &
				...
				+ RedWin%Wing(jstart:TimeSteps%npoints)*(5*RedWin%dlog10Ndm &
				+ tmp2(jstart:TimeSteps%npoints)/hubble_tmp(jstart:TimeSteps%npoints)**2 &
				- RedWin%comoving_density_ev(jstart:TimeSteps%npoints)/hubble_tmp(jstart:TimeSteps%npoints))
			
			!this strategy makes back_count_tmp redundant
